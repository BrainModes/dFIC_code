...
'''The following script allows to run Jansen-Rit simulation with dynamic Feedback Inhibiton Control algorithm 
for chosen GC and y_0^target values. It consititutes the necessary basis to replicate the findings from:
    
'Homeodynamic feedback inhibition control in whole-brain simulations'

and for adjusting the code to individual needs.
...
your_y0_target = float(0.01) # target for the FIC algorithm
...
# Global coupling value (G)
gc_val = numpy.array([10])
detORstoch = 'det' # 'stoch' or 'det' to turn on the noise during tuning

# SEtting up the BOLD monitor
...
# Setting up the sigmoidal function for the Jansen-Rit model
...
# helper function to find the mu for the given target
...
# helper function to find the initial conditions for the given target
...
# the main function to run simulations
...
### Defining the main funtion to run post FIC simulations
...
## running the post-FIC simulations
if run_post_fic:
