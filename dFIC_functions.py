...
documented with help if Chat GPT 3.5
"""
...
def get_peak_freq(PSP, sampling_freq=1000, nperseg=2048):
    ...
    Parameters:
        ...
        sampling_freq (int, optional): Sampling frequency. 
        nperseg (int, optional): Length of each segment.
...
def BandPass(fs, lowcut=None, highcut=None, order=5, filt=sig.butter):
    ...
    Parameters:
        ...
        filt (function, optional): Filter function.
...
def regimes_counter(pp, cut_off=6, perc_td=7.5):
    ...
    cut_off (int, optional): A threshold value to determine 'FP', 'SLC', and 'FLC'.
    perc_td (int, optional): The percentage threshold to identify different regimes in the time series. 
...
# It also counts the number of regimes per node based on a specified cut-off line.
    ...
    for idx, n in enumerate(node_order[:N_nodes:1]):
        ...
        im = ax.scatter(poincare_points[:-1], poincare_points[1:], s=40, marker ='.',color=cmap84(n), alpha=0.55)
        ax.set_xlabel('$PSP_{{i,k}}$')
        ax.set_ylabel('$PSP_{{i,k+1}}$')
...
def FC_bootstrapping(r_dict, window_sizes=[1200, 1600, 2000], step_size=3, verbose=False):
    ...
    Parameters:
        ...
        window_sizes (list, optional): Window sizes for bootstrapping. 
        step_size (int, optional): Step size for sliding the window.
